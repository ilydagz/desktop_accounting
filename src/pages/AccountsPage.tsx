...
import { Printer, Plus, User, Building2, ChevronDown, ChevronUp, Check, ChevronsUpDown, UserPlus } from "lucide-react"
...
export type Account = {
  id: string; type: "individual" | "corporate" | "owner"; name: string; phone: string; email: string; city?: string; district?: string; tax_number?: string; tax_office?: string; land_share?: string; block_number?: string; parcel?: string; flat_count?: string; owner_id?: string | null; currency?: string; borc: number; alacak: number; bakiye: number; created_at?: string; 
}
...
export default function AccountsPage() {
  ...
  const handlePrintAccountsList = () => {
    ...
    const visibleAccounts = accounts.filter(a => a.type !== 'owner');
  ...
  const handleEdit = (account: Account) => {
      ...
      setFormData({
          ad, soyad, unvan, tc: account.tax_number || "", vkn: account.tax_number || "", vergiDairesi: account.tax_office || "",
          telefon: phone, email: account.email || "", city: account.city || "", district: account.district || "",
          arsaPayi: account.land_share || "", adaParsel: account.parcel || "", blokNo: account.block_number || "", daireSayisi: account.flat_count || "",
  ...
  const openNewAccountModal = () => {
      setEditingId(null); 
      setFormData({ ad: "", soyad: "", unvan: "", tc: "", vkn: "", vergiDairesi: "", telefon: "", email: "", city: "", district: "", arsaPayi: "", adaParsel: "", blokNo: "", daireSayisi: "", ownerId: "none", currency: "TRY", borc: "", alacak: "", bakiye: "" });
      setShowNewAccount(true);
  }

  const handleSave = async () => {
    const displayName = accountType === "individual" ? `${formData.ad} ${formData.soyad}` : formData.unvan
    if (!displayName.trim()) { toast({ title: "Hata", description: "Lütfen Ad/Soyad veya Ünvan giriniz.", variant: "destructive" }); return }
    ...
    try {
        const commonData = {
            ...
            email: formData.email, city: formData.city, district: formData.district,
            land_share: formData.arsaPayi, block_number: formData.blokNo, parcel: formData.adaParsel, flat_count: formData.daireSayisi,
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-3xl font-bold tracking-tight">Cariler</h1><p className="text-muted-foreground mt-1">Müşteri ve tedarikçi hesapları.</p></div>
        <div className="flex gap-3">
            <Button variant="outline" onClick={handlePrintAccountsList} className="gap-2"><Printer className="h-4 w-4" /> Listeyi Yazdır</Button>
      ...
      <Dialog open={showNewAccount} onOpenChange={setShowNewAccount}>
        <DialogContent className="sm:max-w-[650px] max-h-[90vh] overflow-y-auto">
          <DialogHeader><DialogTitle>{editingId ? "Cari Düzenle" : "Yeni Cari Hesap"}</DialogTitle></DialogHeader>
          ...
          <div className="grid gap-5 py-2">
            ...
            {accountType === 'individual' ? (
                <div className="grid gap-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <div className="space-y-1.5"><Label>TC Kimlik No</Label><Input autoComplete="off" value={formData.tc} onChange={e => setFormData({...formData, tc: e.target.value})} maxLength={11} placeholder="11 haneli TC Kimlik No" /></div>
                </div>
            ) : (
                <div className="grid gap-4 animate-in fade-in slide-in-from-top-2">
                    <div className="space-y-1.5"><Label>Ünvan (Kurum / Site / Apartman Adı)</Label><Input autoComplete="off" value={formData.unvan} onChange={e => setFormData({...formData, unvan: e.target.value})} placeholder="Örn: Güneş Apartmanı Yönetimi" /></div>
            ...
            {accountType === 'corporate' && (
                <div className="border rounded-lg overflow-hidden animate-in fade-in slide-in-from-top-2">
                    <button type="button" onClick={() => setShowBuildingInfo(!showBuildingInfo)} className="w-full flex items-center justify-between p-3 bg-muted/50 hover:bg-muted transition-colors"><span className="text-sm font-medium">Yapı, Arsa ve Apartman Özellikleri</span>{showBuildingInfo ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}</button>
                    {showBuildingInfo && (
